...
<div class="space-y-6 sm:space-y-8">
    ...
    <!-- Header Section -->
    <div
        class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 bg-white p-5 sm:p-6 rounded-2xl sm:rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a]">
        <div>
            <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2.5 sm:gap-3">
                <span
                    class="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-blue-600 rounded-full inline-block border border-slate-900"></span>
                Kelola Tautan
            </h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-1 sm:mt-1.5">Atur tautan bio Anda dengan gaya modern
                interaktif.</p>
        </div>
    ...
    <!-- Data List Container -->
    <div
        class="bg-white rounded-2xl sm:rounded-3xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] sm:shadow-[6px_6px_0px_0px_#0f172a] overflow-hidden flex flex-col">

        <!-- Table Header (Desktop Only) -->
        <div
            class="hidden lg:grid grid-cols-12 gap-4 bg-slate-100 text-slate-900 px-6 py-4 border-b-2 border-slate-900 text-xs font-black uppercase tracking-wider">
        ...
        <div class="divide-y-2 divide-slate-900 bg-white">
            ...
            <div
                class="flex flex-col lg:grid lg:grid-cols-12 gap-4 lg:gap-4 items-start lg:items-center p-4 sm:p-6 lg:p-6 hover:bg-blue-50/60 transition-colors group">
                ...
                <div class="lg:col-span-5 flex items-center space-x-3 sm:space-x-4 w-full">
                    ...
                    @else
                    <div
                        class="flex-shrink-0 h-10 w-10 sm:h-12 sm:w-12 bg-amber-200 text-slate-900 font-black border-2 border-slate-900 flex items-center justify-center rounded-lg sm:rounded-xl shadow-[2px_2px_0px_0px_#0f172a]">
                    ...
                    <div class="overflow-hidden">
                        <div
                            class="text-sm sm:text-base font-extrabold text-slate-900 group-hover:text-blue-600 transition-colors truncate">
                            {{ $link->title }}
                        </div>
                ...
                <div class="flex flex-row lg:contents w-full gap-4 mt-2 lg:mt-0">
                    ...
                    <div class="lg:col-span-2 flex flex-col lg:flex-row items-start lg:items-center flex-1">
                        <span
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 lg:hidden">Status</span>
                        @if($link->is_active)
                        <span
                            class="px-2.5 py-1 inline-flex text-xs font-extrabold rounded-md sm:rounded-lg bg-emerald-200 text-emerald-900 border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] items-center gap-1.5 whitespace-nowrap">
                            <span class="w-1.5 h-1.5 bg-emerald-600 rounded-full border border-slate-900"></span> Aktif
                        </span>
                        @else
                        <span
                            class="px-2.5 py-1 inline-flex text-xs font-extrabold rounded-md sm:rounded-lg bg-rose-200 text-rose-900 border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] items-center gap-1.5 whitespace-nowrap">
                    ...
                    <div class="lg:col-span-3 flex flex-col lg:flex-row items-start lg:items-center flex-1">
                        <span
                            class="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 lg:hidden">Statistik</span>
                        <div
                            class="inline-flex items-center px-2.5 py-1 rounded-md sm:rounded-lg bg-slate-100 border border-slate-300 text-xs font-extrabold text-slate-800 whitespace-nowrap">
                ...
                <div
                    class="lg:col-span-2 flex items-center justify-start lg:justify-end space-x-2 w-full lg:w-auto mt-2 lg:mt-0 pt-4 lg:pt-0 border-t-2 border-dashed border-slate-200 lg:border-none">
            ...
            <div class="px-6 py-16 text-center">
                <div
                    class="flex flex-col items-center justify-center max-w-sm mx-auto p-6 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-400">
                    <div
                        class="bg-amber-200 p-3 rounded-2xl border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a] mb-3 text-slate-900">
